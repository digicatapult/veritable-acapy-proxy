...
function check_versions_consistent () {
  ...
  local HELM_VALUES_TAG_VERSION=$(yq eval '.image.tag' ./helm/vitalam-service-template/values.yaml)
  local HELM_CHART_VERSION=$(yq eval '.version' ./helm/vitalam-service-template/Chart.yaml)
  local HELM_CHART_APP_VERSION=$(yq eval '.appVersion' ./helm/vitalam-service-template/Chart.yaml)
